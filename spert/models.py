...
class SpERT(BertPreTrainedModel):
    ...
    def load_src(self, src_path, start_layer=11, end_layer=11):
        src_dict = torch.load(src_path)
        bert_dict = self.bert.state_dict()
        src_bert = dict()
        target_list = []
        for i in range(start_layer, end_layer+1):
            target_list.append(str(i))
        for k,v in src_dict.items():
            if k.startswith("bert") and self.check(target_list, k):
                src_bert[k[5:]] = v
                print(k)
        bert_dict.update(src_bert)
        self.bert.load_state_dict(bert_dict)
        # self.size_embeddings.weight.data.copy_(src_dict["size_embeddings.weight"])
        print("transfer finished")
    
    def check(self, target_list, key):
