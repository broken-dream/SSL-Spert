...
def convert_predictions_semi(batch_entity_clf: torch.tensor, batch_rel_clf: torch.tensor,
    ...
    # print("batch ner num:{}   batch rel num:{}".format(len(batch_pred_entities), len(batch_pred_relations)))
    # for item in batch_pred_entities:
    #     print(item)
...
def _convert_pred_relations(rel_clf: torch.tensor, rels: torch.tensor,
    ...
    for i in range(pred_rel_types.shape[0]):
        ...
        # remove none entity
        if pred_head_type_idx == 0 or pred_tail_type_idx == 0:
            continue
...
def store_predictions(documents, pred_entities, pred_relations, store_path):
    ...
    for i, doc in enumerate(documents):
        ...
        # print("ent num:{}  rel num:{}".format(len(converted_entities), len(converted_relations)))
        # print("-------------------------")
...
def store_predictions_semi(documents, pred_entities, pred_relations, store_path):
    predictions = []
    ...
    # store as json
    # with open(store_path, 'a+') as predictions_file:
